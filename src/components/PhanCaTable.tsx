...
export default function PhanCaTable() {
  ...
  const [lastSaved, setLastSaved] = useState<string | null>(() => localStorage.getItem('PHAN_CA_LAST_SAVED'));
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      const localDataKey = `PHAN_CA_DATA_${userProfile.ma_kho}`;
      ...
      if (localData) {
        try {
          ...
          if (parsed[monthKey] && Array.isArray(parsed[monthKey]) && parsed[monthKey].length > 0) {
            setEmployees(parsed[monthKey]);
            setSaveMessage({ type: 'success', text: 'Đã tải dữ liệu từ trình duyệt!' });
            setLoading(false);
            return;
          }
        } catch (e) {
          console.error('Error parsing local phan ca data:', e);
        }
      }
      ...
      if (lkData?.data_phan_ca) {
        try {
          const parsed = typeof lkData.data_phan_ca === 'string' 
            ? JSON.parse(lkData.data_phan_ca) 
            : lkData.data_phan_ca;
          ...
          if (Array.isArray(parsed)) {
            rawEmployees = parsed;
            hasShiftData = rawEmployees.length > 0;
          } else if (typeof parsed === 'object' && parsed !== null) {
            rawEmployees = parsed[monthKey] || [];
            hasShiftData = rawEmployees.length > 0;
          }
        } catch (e) {
          console.error('Error parsing data_phan_ca:', e);
        }
      }

      // If no shift data for this month, try to initialize from staff list (ds_nhan_vien)
      if (!hasShiftData && lkData?.ds_nhan_vien) {
        try {
      ...
      if (rawEmployees.length > 0) {
        const formattedEmployees = rawEmployees.map(emp => ({
          ...
          fullId: emp.fullId || '',
          department: emp.department || 'BP All In One - ĐMX',
          shiftType: emp.shiftType || '',
          shifts: emp.shifts || {}
        }));

        setEmployees(formattedEmployees);
        setSaveMessage({ type: 'success', text: hasShiftData ? 'Đã tải dữ liệu phân ca!' : 'Đã tải danh sách nhân viên!' });
      } else {
        setEmployees([]);
        setSaveMessage({ type: 'info', text: 'Chưa có dữ liệu cho tháng này.' });
      }
    } catch (err) {
  ...
  useEffect(() => {
    fetchEmployees();
  }, [userProfile?.ma_kho, currentMonthStart]);
  ...
  const handleUpdateShift = (username: string, date: Date, shiftIndex: number, value: string) => {
    ...
    setEmployees(prev => prev.map(emp => {
      if (emp.username === username) {
        ...
        // Clone the day object to avoid direct mutation
  ...
  const handleToggleOff = (username: string, date: Date) => {
    ...
    setEmployees(prev => prev.map(emp => {
      if (emp.username === username) {
        ...
        // If turning OFF, clear all shifts
        if (dayShifts.isOff) {
  ...
  const handleDrop = (e: React.DragEvent, targetUsername: string) => {
    ...
    setEmployees(prev => {
      ...
      // Update department if dragged to a different department group
      draggedEmployee.department = targetEmployee.department;

      // Remove dragged employee
      newEmployees.splice(draggedIdx, 1);
      
      // Find new target index (since array mutated)
  ...
  const handleAutoAssign = () => {
    setEmployees(prev => {
      // Deep clone to avoid direct mutation of objects in state
      ...
      // Bắt đầu xoay vòng từ nhân viên đầu tiên có shiftType là Hành Chính
      let currentIndex = initialHcIndices[0];

      monthDates.forEach((date, dayIdx) => {
        const dateStr = format(date, 'yyyy-MM-dd');
        let hcPairUsernames: string[] = [];
        let selectedIndices: number[] = [];
        
        // Domino logic: tìm đủ N nhân viên cho ngày hôm nay, bỏ qua người OFF
        while (selectedIndices.length < Math.min(N, totalEmp)) {
          ...
          while (attempts < totalEmp) {
            ...
            // Kiểm tra nếu nhân viên chưa được chọn hôm nay VÀ không có lịch OFF
            ...
            // Domino: Nếu trùng lịch OFF hoặc đã chọn, nhảy sang người kế tiếp
            currentIndex++;
            attempts++;
          }
        ...
        newEmployees.forEach(emp => {
          // Clone day object for immutable update
    ...
    setSaveMessage({ type: 'success', text: 'Đã chia ca tự động (Domino: Ưu tiên đủ nhân sự Hành chính)!' });
  };

  const calculateWeeklyTotal = (emp: any) => {
    let total = 0;
    Object.values(emp.shifts || {}).forEach((dayShifts: any) => {
      Object.values(dayShifts).forEach((val: any) => {
        total += (parseFloat(val as string) || 0);
      });
    });
    return total;
  };
    ...
    try {
      ...
      if (localDataStr) {
        try {
          ...
          console.error('Error parsing local data for save:', e);
        }
      }
      
      // Update current month
      allData[monthKey] = employees;
      ...
      setLastSaved(now);
      localStorage.setItem('PHAN_CA_LAST_SAVED', now);
      setSaveMessage({ type: 'success', text: 'Đã lưu dữ liệu vào trình duyệt!' });
    } catch (err: any) {
      console.error('Lỗi khi lưu dữ liệu:', err);
      setSaveMessage({ type: 'error', text: 'Lỗi lưu dữ liệu: ' + (err.message || 'Unknown error') });
    } finally {
      setIsSaving(false);
      setTimeout(() => setSaveMessage({ type: '', text: '' }), 3000);
    }
    ...
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="bg-white text-slate-800 px-4 py-3 flex items-center justify-between sticky top-0 z-50 shadow-md border-b border-slate-200 flex-wrap gap-4">
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-4xl font-black uppercase tracking-wider leading-tight text-[#004b8d] whitespace-nowrap">LỊCH PHÂN CA THÁNG</h1>
          
          <div className="flex items-center bg-slate-100 hover:bg-slate-200 rounded-lg px-2 py-1.5 gap-3 cursor-pointer transition-all border border-slate-200 shadow-inner text-slate-700">
            <button onClick={() => setCurrentMonthStart(d => addMonths(d, -1))} className="hover:bg-white p-1 rounded transition-colors shadow-sm">
              <ChevronLeft size={16} />
            </button>
            <span className="text-xs font-bold uppercase w-24 text-center leading-tight tracking-wide">
              THÁNG<br/>{format(currentMonthStart, 'MM/yyyy')}
            </span>
            <button onClick={() => setCurrentMonthStart(d => addMonths(d, 1))} className="hover:bg-white p-1 rounded transition-colors shadow-sm">
          ...
          {saveMessage.text && (
            <div className={`text-[10px] font-bold uppercase px-3 py-1.5 rounded-lg flex items-center min-h-[48px] shadow-sm border whitespace-normal max-w-[200px] leading-tight ${saveMessage.type === 'success' ? 'bg-[#008080] text-white border-[#006666]' : 'bg-red-600 text-white border-red-700'}`}>
              {saveMessage.text}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          <button
            ...
            className="bg-gradient-to-b from-sky-500 to-sky-600 hover:from-sky-600 hover:to-sky-700 border border-sky-400 px-2 py-1.5 rounded-lg text-[11px] font-bold uppercase transition-all flex items-center justify-center gap-1.5 shadow-md text-white min-h-[48px] w-[110px] leading-tight text-center whitespace-normal hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0 active:shadow-md"
          >
            <RefreshCw size={14} className={`shrink-0 ${loading ? 'animate-spin' : ''}`} /> Tải lại
          </button>

          <label className="cursor-pointer bg-gradient-to-b from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 border border-slate-500 px-2 py-1.5 rounded-lg text-[11px] font-bold uppercase transition-all flex items-center justify-center gap-1.5 shadow-md text-white min-h-[48px] w-[110px] leading-tight text-center whitespace-normal hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0 active:shadow-md">
            <Trash2 size={14} className="rotate-180 shrink-0" /> Nhập Excel
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={isSaving}
                className="bg-gradient-to-b from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 border border-emerald-400 px-3 py-2 rounded-lg text-[12px] font-bold uppercase transition-all flex items-center justify-center gap-2 shadow-md text-white min-h-[48px] min-w-[130px] leading-tight text-center whitespace-normal disabled:opacity-50 disabled:hover:translate-y-0 hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0 active:shadow-md"
              >
            ...
            <thead>
              ...
              <tr className="bg-[#e6f0fa] border-b border-slate-300">
                ...
                <th rowSpan={2} className="sticky left-64 z-40 bg-[#e6f0fa] border-r border-slate-300 px-2 py-2 text-center w-24 min-w-[96px] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  <span className="font-black text-[#004b8d] uppercase leading-tight block text-[10px]">Giờ công<br/>trong tuần</span>
                </th>
              ...
              <tr className="bg-[#f0f5fa] border-b border-slate-300">
                {monthDates.map((date, i) => {
                  ...
                  return SHIFTS.map((shift, sIdx) => (
                    <th key={`${i}-${sIdx}`} className={`px-1 py-1 text-[9px] font-bold uppercase w-12 min-w-[48px] ${sIdx === 4 ? 'border-r-2 border-slate-400' : 'border-r border-slate-200'} ${isWeekend ? 'text-red-500/80 bg-red-50/30' : 'text-slate-500'}`}>
                      {shift.replace('Ca ', 'C')}
                    </th>
                  ));
                })}
              ...
              {Object.entries(groupedEmployees).map(([dept, deptEmps]) => (
                <React.Fragment key={dept}>
                  ...
                  <tr className="bg-[#f1f5f9]">
                    <td colSpan={2 + monthDates.length * 5} className="sticky left-0 z-20 bg-[#f1f5f9] px-4 py-2 font-black text-[#004b8d] uppercase border-b border-slate-300 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                      {dept}
                    </td>
                  </tr>
                  {deptEmps.map((emp) => (
                    ...
                    >
                      ...
                      {monthDates.map((date, i) => {
                        ...
                        // Check if this is a full "Hành Chính" day (all 5 shifts have values)
                        ...
                        return (
                          <td 
                            key={i}
                            colSpan={5} 
                            className={`border-r-2 border-slate-400 p-0 relative transition-colors ${isOff ? 'bg-red-50' : ''}`}
                          >
                            <div className="flex flex-col h-full">
                              {/* OFF Toggle Button */}
                              <button
                                onClick={() => handleToggleOff(emp.username, date)}
                                className={`absolute top-0 right-0 z-10 px-1 py-0.5 text-[8px] font-bold uppercase transition-colors rounded-bl border-l border-b border-slate-200 ${
                                  isOff 
      ...
      <div ref={summaryTableRef} className="mt-8 bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden mx-4 mb-8">
        <div className="bg-[#004b8d] px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-white font-bold uppercase tracking-wider text-sm">Tóm tắt nhân viên đi ca hành chính</h2>
          </div>
          <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {employees.map(emp => {
                const hcDays = monthDates.filter(date => {
                  const dateStr = format(date, 'yyyy-MM-dd');
              ...
              {employees.length > 0 && employees.every(emp => {
                ...
                <tr>
                  <td colSpan={3} className="px-4 py-8 text-center text-slate-400 italic text-sm">
                    Không có nhân viên nào đi ca hành chính trong tháng này.
                  </td>
                </tr>
              )}
            </tbody>
