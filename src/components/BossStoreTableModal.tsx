...
import { 
  ...
  FileSpreadsheet, ArrowLeft, ArrowRight, Store, AlertCircle, Sparkles
} from 'lucide-react';
...
interface BossStoreTableModalProps {
  ...
  rows: BossStoreItem[];
  headers?: string[];
  onUpdateRow: (id: string, newMaKho: string, newTenSieuThi: string) => Promise<void>;
  onDeleteRow: (id: string) => Promise<void>;
  onAddRow: (maKho: string, tenSieuThi: string) => Promise<void>;
...
export const BossStoreTableModal: React.FC<BossStoreTableModalProps> = ({
  ...
  const handleSaveEdit = async (id: string) => {
    if (!editMaKho.trim() || !editTenSieuThi.trim()) {
      setActionError('Mã kho và Tên siêu thị không được để trống!');
      return;
    }
    setActionError(null);
    try {
      await onUpdateRow(id, editMaKho.trim(), editTenSieuThi.trim());
      setEditingId(null);
    } catch (err: any) {
  ...
  const handleSaveNewRow = async () => {
    if (!newMaKho.trim() || !newTenSieuThi.trim()) {
      setActionError('Vui lòng nhập đầy đủ Mã kho và Tên siêu thị!');
      return;
    }
    setActionError(null);
    try {
      await onAddRow(newMaKho.trim(), newTenSieuThi.trim());
      setNewMaKho('');
      setNewTenSieuThi('');
      setIsAdding(false);
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-2 sm:p-4 bg-slate-950/70 backdrop-blur-sm">
      <motion.div
        ...
        className="w-full max-w-5xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300 font-medium">
                Tổng cộng: <strong className="text-amber-400 font-black">{rows.length}</strong> siêu thị được lưu trên Firebase
              </p>
            </div>
          </div>
        ...
        <div className="p-3.5 sm:p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between shrink-0">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder="Tìm theo mã kho (Cột E) hoặc tên siêu thị (Cột C)..."
        ...
        {/* Form Add New Row */}
        {isAdding && (
          <div className="p-3.5 bg-emerald-50/70 border-b border-emerald-200 flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
            ...
            <table className="w-full text-left border-collapse text-xs table-fixed">
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
                {paginatedRows.map((row, index) => {
                  ...
                  const isMatchCurrent = currentMaKho && String(row.maKho || '').trim().replace(/^0+/, '') === String(currentMaKho).trim().replace(/^0+/, '');

                  if (isEditing) {
                    return (
                      <tr key={row.id} className="bg-amber-50/70">
                        <td className="py-2 px-3 text-center font-bold text-slate-400">{stt}</td>
                  ...
                  return (
                    <tr 
                      ...
                      className={`hover:bg-indigo-50/40 transition-colors ${
                        isMatchCurrent ? 'bg-indigo-50/60 font-bold' : ''
                      }`}
                    >
                      <td className="py-2.5 px-3 text-center text-slate-400 font-bold">{stt}</td>
                      <td className="py-2.5 px-3 font-bold">
                        <span className={`inline-block px-2.5 py-0.5 rounded-md text-[11px] font-black tracking-wide ${
                          isMatchCurrent 
        ...
        <div className="px-5 py-3 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
          <div className="text-xs font-bold text-slate-500">
            ...
            {onlyCurrentKho && <span className="text-indigo-600 ml-1">(đang lọc theo kho {currentMaKho})</span>}
          </div>
