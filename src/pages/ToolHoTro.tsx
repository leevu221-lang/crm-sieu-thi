...
export default function ToolHoTro() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] font-sans">
      {/* Top Header Section - CRM SIÊU THỊ Modern Topbar */}
      <header className="bg-white border-b border-slate-200/90 px-6 py-3 sticky top-0 z-30 shadow-xs">
        <div className="max-w-[1800px] mx-auto flex items-center justify-between gap-4 flex-wrap">
          
          {/* CỤM TRÁI: LOGO + CRM SIÊU THỊ + CHỌN SIÊU THỊ */}
          <div className="flex items-center gap-4">
            ...
            >
              <ChevronLeft className={`w-4 h-4 transition-transform duration-200 ${!isSidebarExpanded ? 'rotate-180' : ''}`} />
            </button>

            <div className="h-6 w-px bg-slate-200 mx-1 hidden lg:block"></div>

            {/* Viên thuốc chọn Siêu Thị */}
            <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-[#00965e] hover:bg-[#008252] text-white font-extrabold text-xs tracking-tight shadow-xs transition-all cursor-pointer">
              <Store className="w-4 h-4 text-emerald-100 stroke-[2.2]" />
              <span className="truncate max-w-[260px] md:max-w-[320px]">{userProfile?.ten_sieu_thi || `ĐML_CMA_CMA - Kho ${maKho || '43751'}`}</span>
              <ChevronDown className="w-3.5 h-3.5 text-emerald-200" />
            </div>
          </div>

          {/* CỤM PHẢI: ĐỒNG HỒ REALTIME + TÀI KHOẢN ADMIN (ĐỒNG BỘ THEO PHONG CÁCH HÌNH 2) */}
          <div className="flex items-center gap-3">
            
            {/* 1. Viên Thuốc Thời Gian (Nền vàng kem, chữ & icon nâu ấm chuẩn Hình 2) */}
            ...
            {/* 2. Viên Thuốc Tài Khoản (Nền vàng kem, icon User & ChevronDown chuẩn Hình 2) */}
            <div className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full bg-[#FFF7ED] border border-[#FFEDD5] text-[#9A3412] hover:bg-[#FFEDD5] hover:border-[#FED7AA] transition-all shadow-2xs cursor-pointer">
              <User className="w-4 h-4 text-[#C2410C] stroke-[2.2]" />
              <span className="text-sm font-extrabold tracking-tight">{maKho || '43751'} - {userProfile?.role || 'admin'}</span>
              <ChevronDown className="w-4 h-4 text-[#C2410C] stroke-[2.2] ml-0.5" />
            </div>
