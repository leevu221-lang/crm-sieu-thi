...
export default function NewRealtimePage({ pageMaintenanceState = {}, isUser43751Local = false, pageHiddenState = {} }: { pageMaintenanceState?: Record<string, boolean>, isUser43751Local?: boolean, useV2Layout?: boolean, pageHiddenState?: Record<string, boolean> }) {
  ...
  const isUser43751 = String(userProfile?.username || '').trim() === '43751' || 
                      ...
                      (typeof window !== 'undefined' && localStorage.getItem('rtst_ma_kho') === '43751');
  ...
  const forceDesktopLayout = (element: HTMLElement) => {
    ...
    const cardsGrid = element.querySelector('.grid-cols-2.md\\:grid-cols-3.lg\\:grid-cols-3.xl\\:grid-cols-6') ||
                      element.querySelector('.grid-cols-2.sm\\:grid-cols-3.lg\\:grid-cols-6') ||
                      element.querySelector('[class*="grid-cols-2"][class*="grid-cols-6"]');
    if (cardsGrid) {
                      ...
                      <>
                        ...
                        <div ref={categoriesRef} className="w-full">
                          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3.5 items-start">
                            ...
                            <div
                              ref={categorySLRef}
                              className={`bg-white/95 rounded-2xl overflow-hidden min-w-0 flex flex-col p-1 sm:p-2.5 self-start h-auto ${
                                isEffective43751
                                  ? 'border border-[#BAE6FD]/80 shadow-[0_10px_30px_-5px_rgba(2,132,199,0.08),0_4px_6px_-2px_rgba(0,0,0,0.03)]'
                            ...
                            >
                              ...
                              <div
                                ...
                                } p-2 sm:p-4 rounded-xl sm:rounded-2xl text-white relative shrink-0 mb-1.5 sm:mb-2.5 shadow-md`}
                              >
                                        ...
                                        return (
                                          <tr key={idx} className={`${isEven ? 'bg-white' : (isEffective43751 ? 'bg-sky-50/25' : 'bg-emerald-50/20')} ${isEffective43751 ? 'hover:bg-sky-50/70' : 'hover:bg-emerald-50/70'} transition-colors h-[28px] sm:h-[36px] md:h-[40px]`}>
                                            ...
                                            {showOrangeCols && (() => {
                                              ...
                                              if (!lkCat || lkCat.target === 0) return <td className={`px-0.5 py-0.5 text-[8.5px] sm:text-[11.5px] md:text-[13.5px] font-black text-center border-b ${isEffective43751 ? 'border-sky-100/90' : 'border-emerald-100/90'} text-slate-400 whitespace-nowrap overflow-hidden`}></td>;
                                              const mucTieu = Math.round((lkCat.target / mucTieu100Info.totalDaysInMonth) * mucTieu100Info.daysPassed - lkCat.revenue);
                                              return <td className={`px-0.5 py-0.5 text-[8.5px] sm:text-[11.5px] md:text-[13.5px] font-black text-center border-b ${isEffective43751 ? 'border-sky-100/90' : 'border-emerald-100/90'} whitespace-nowrap overflow-hidden ${mucTieu > 0 ? 'text-rose-600' : (isEffective43751 ? 'text-sky-700' : 'text-emerald-600')}`}>{mucTieu > 0 ? Math.round(mucTieu).toLocaleString() : ''}</td>;
                                            })()}
                                          </tr>
                                        );
                                      }))}
                                        ...
                                        return (
                                          <tr key={idx} className={`${isEven ? 'bg-white' : (isEffective43751 ? 'bg-sky-50/25' : 'bg-blue-50/25')} ${isEffective43751 ? 'hover:bg-sky-50/70' : 'hover:bg-blue-50/70'} transition-colors h-[28px] sm:h-[36px] md:h-[40px]`}>
                                            ...
                                            {showOrangeCols && (() => {
                                              ...
                                              if (!lkCat || lkCat.target === 0) return <td className={`px-0.5 py-0.5 text-[8.5px] sm:text-[11.5px] md:text-[13.5px] font-black text-center border-b ${isEffective43751 ? 'border-sky-100/90' : 'border-blue-100/90'} text-slate-400 whitespace-nowrap overflow-hidden`}></td>;
                                              const mucTieu = Math.round((lkCat.target / mucTieu100Info.totalDaysInMonth) * mucTieu100Info.daysPassed - lkCat.revenue);
