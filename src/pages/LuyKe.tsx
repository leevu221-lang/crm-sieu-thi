...
const LuyKe: React.FC<{ pageMaintenanceState?: Record<string, boolean>, isUser43751Local?: boolean }> = ({ pageMaintenanceState = {}, isUser43751Local = false }) => {
    ...
    if (storeIdx === -1 || categoryIdx === -1) {
      ...
      sampleRows.forEach(row => {
        ...
        for (let c = 0; c < Math.min(row.length, 30); c++) {
          ...
          const matchesMarket = (availableMarketsList || []).some(m => {
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (files.length >= 2) {
        ...
        const hasExistingTodayData = selectedStoreExcelData.rows && selectedStoreExcelData.rows.length > 0;
        ...
        if (hasExistingTodayData) {
          ...
          saveExcelThuongStData(
            ...
            resNew.detectedLimit,
            filteredMarkets.map((m: any) => m.name),
            selectedStoreExcelData.rows, // Today becomes Yesterday
  ...
  const selectedStoreExcelData = React.useMemo(() => {
    ...
    if (storeTarget) {
      return {
        ...
        oldRows: storeTarget.thuongStOldRows || [],
        oldFileName: storeTarget.excelOldFileName || '',
        oldLimit: storeTarget.topPercentRankLimitOld ?? 7
      };
    }
    ...
    return {
      ...
      oldRows: thuongStOldRows || [],
      oldFileName: excelOldFileName || '',
      oldLimit: topPercentRankLimitOld ?? 7
    };
  }, [allStoreTargets, selectedStoreFilter, thuongStRows, excelFileName, topPercentRankLimit, thuongStOldRows, excelOldFileName, topPercentRankLimitOld]);
  ...
  const isExcelActive = React.useMemo(() => {
    if (excelFileName || excelOldFileName) return true;
  ...
  }, [allStoreTargets, filteredMarkets, excelFileName, excelOldFileName, selectedStoreExcelData]);
