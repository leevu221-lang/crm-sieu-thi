...
export const ClusterReportTab: React.FC<ClusterReportTabProps> = ({
  ...
  const { rows, summaryRow, kpiHeader, has11Cols } = useMemo(() => {
    // 1. First attempt: Parse raw text from LUỸ KẾ DT
    let parsed = rawInput ? parseClusterSummaryData(rawInput) : { rows: [], summaryRow: null, kpiHeader: null, has11Cols: false };

    // If parsed gave valid rows with non-zero metrics, return them
    if (parsed.rows.length > 0 && !isAllZeroDummy(parsed.rows)) {
      return parsed;
    }

    // 2. Second attempt: Fallback to clusterMarkets (parsed by LuykeDataContext from LUỸ KẾ DT / lk_bi_tong_quan)
    ...
    // 3. Fallback: If rows are all zero dummy, return empty state
    if (isAllZeroDummy(parsed.rows)) {
      return { rows: [], summaryRow: null, kpiHeader: null, has11Cols: false };
    }
  ...
  const handleSavePasteData = () => {
    ...
    const textToSave = pasteInputText.trim();
    setLocalRawOverride(textToSave);
    try {
      localStorage.setItem('rt_catrev', textToSave);
      localStorage.setItem('rtst_cluster_summary', textToSave);
      localStorage.setItem('rtst_catrev', textToSave);
    } catch {}
    if (onSaveClusterData) {
      onSaveClusterData(textToSave);
    }
