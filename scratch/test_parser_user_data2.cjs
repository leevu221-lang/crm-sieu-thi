...
function parseMwgBiStaffRevenue(text) {
  ...
  // Find EXACT header block: line "NHÂN VIÊN" followed by "SỐ LƯỢNG"
  let headerIndex = -1;
  ...
  if (headerIndex !== -1) {
    ...
    while (cursor < lines.length) {
      ...
      // Stop or footer conditions
      if (line.includes('1-') && line.includes('/ Tổng')) {
        break;
      }
      if (line.includes('Đơn vị:')) {
        break;
      }

      // Check if this is the "Tổng" row
      ...
      // Check if line is tab separated
      if (line.includes('\t')) {
...
console.log('Sample Staff Last:', res.staffRows[res.staffRows.length - 1]);
